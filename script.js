...
function openPDF(url) {
    ...
    if (url.includes("drive.google.com")) {
        ...
        if (fileId) {
            finalUrl = `https://docs.google.com/viewer?embedded=true&url=https://drive.google.com/uc?id=${fileId}`;
        }
    } else {
        finalUrl = `https://docs.google.com/viewer?embedded=true&url=${encodeURIComponent(url)}`;
    }
    ...
    // আইফ্রেমের সোর্স লোড করার আগে একটি লোডিং এনিমেশন দিন
    fullView.innerHTML = `
        <div class="fixed inset-0 bg-white z-[9999] flex flex-col">
            <div class="p-4 bg-white border-b flex justify-between items-center shadow-md">
                <button onclick="closeFullPost()" class="px-5 py-2 bg-red-50 text-red-600 rounded-xl font-black active:scale-95 transition">✕ বন্ধ করুন</button>
                <span class="text-sm font-black text-indigo-600 uppercase tracking-widest">PDF Reader</span>
                <div class="w-10"></div>
            </div>
            <div class="flex-1 bg-slate-100 flex items-center justify-center">
                <div id="pdf-loader" class="absolute font-bold text-slate-400 animate-pulse">ডকুমেন্ট লোড হচ্ছে...</div>
                <iframe src="${finalUrl}" 
