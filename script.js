...
// ১. লাইভ ক্লক ফিক্স (সেকেন্ড অনুযায়ী চলবে)
    ...
    db.ref('siteNotice').on('value', snapshot => {
        ...
        if (typeof data === 'object') {
            Object.values(data).forEach(val => {
                if (typeof val === 'object' && val.text) noticeArray.push(val.text);
                else if (typeof val === 'string') noticeArray.push(val);
            });
        } else {
            noticeArray.push(data);
        }
        
        // সব নোটিশ পাইপ (|) চিহ্ন দিয়ে যুক্ত করা
        marquee.innerHTML = `📢 ${noticeArray.join(' | ')} --- ধূমকেতু এডুকেশনাল হাবে স্বাগতম!`;
    });
}

// ৩. গুগল ট্রান্সলেট (শুধুমাত্র বাংলা ও ইংরেজি)
window.googleTranslateElementInit = function() {
    new google.translate.TranslateElement({
        pageLanguage: 'en', // অ্যাপের অরিজিনাল ভাষা ইংরেজি হলে 'en' দিন
        includedLanguages: 'en,bn', // শুধুমাত্র এই দুটি ভাষা থাকবে
        layout: google.translate.TranslateElement.InlineLayout.SIMPLE,
        autoDisplay: false
    }, 'google_translate_element');
}

// ৪. পেজ ইনিশিয়ালাইজেশন
