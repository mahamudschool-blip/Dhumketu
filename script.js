...
function createCard(item) {
    // অ্যাকশন নির্ধারণ (PDF এর জন্য openPDF ফাংশন যোগ করা হয়েছে)
    const action = item.type === 'mcq_set' ? `openMCQSet('${item.id}', '${item.path}')` : 
                   ...
                   item.type === 'pdf' ? `openPDF('${item.url}')` : // PDF Action Updated
            ...
            <div class="flex flex-wrap gap-2 mb-5">
                ${item.questions ? `
                    <span class="text-sm bg-purple-100 text-purple-700 px-3 py-1.5 rounded-full flex items-center gap-1 font-medium shadow-sm">
                        <span class="text-lg">📊</span> ${item.questions.length} প্রশ্ন
                    </span>` : ''}
...
function closeFullPost() {
    ...
    // হ্যাং হওয়া রোধ করতে ভেতরের আইফ্রেম বা কন্টেন্ট পুরোপুরি মুছে ফেলা
    if (fullView) {
        fullView.innerHTML = ""; 
        fullView.style.display = 'none';
    }
    
    // স্ক্রল স্বাভাবিক করা
    document.body.style.overflow = 'auto';
    
    // মেমোরি ক্লিনআপ (ঐচ্ছিক কিন্তু কার্যকর)
    console.log("View Closed & Cleared");
}
...
// PDF ওপেন করার ফাংশন (লগইন ছাড়া ভিউয়ার সাপোর্টসহ)
/**
 * Professional PDF Viewer with Enhanced Features
 * Supports Google Drive, Direct Links, and Embedded Viewer
 */
function openPDF(url, title = "PDF Document") {
    if (!url) {
        showToast("PDF লিঙ্ক পাওয়া যায়নি!", "error");
        return;
    }

    const { viewUrl, downloadUrl, directUrl } = processPDFUrl(url);
    const fullView = document.getElementById('fullPostView');
    ...
    fullView.innerHTML = `
        <div class="fixed inset-0 bg-gradient-to-br from-slate-900 to-slate-800 z-[9999] flex flex-col animate-fade-in">
            <!-- Header with Controls -->
            <div class="flex flex-col md:flex-row justify-between items-center p-4 md:px-8 bg-white/95 backdrop-blur-sm border-b border-slate-200 shadow-lg">
                <div class="flex items-center gap-4 w-full md:w-auto">
                    <button onclick="closeFullPost()" 
                            class="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-red-500 to-rose-600 text-white rounded-xl font-bold text-sm hover:from-red-600 hover:to-rose-700 active:scale-95 transition-all duration-200 shadow-md hover:shadow-lg">
                    ...
                    <div class="flex-1 md:flex-none">
                        <h3 class="text-sm font-bold text-slate-800 truncate" title="${title}">
                            📄 ${title}
                        </h3>
                        <p class="text-[10px] text-slate-500 truncate" id="pdf-url-display">
                ...
                <div class="flex items-center gap-3 mt-3 md:mt-0">
                    ...
                    <a href="${downloadUrl}" 
                       target="_blank" 
                       download="${title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.pdf"
                       onclick="trackPDFDownload('${title}')"
                       class="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl font-bold text-sm hover:from-green-600 hover:to-emerald-700 active:scale-95 transition-all duration-200 shadow-md hover:shadow-lg group">
                    ...
                    <!-- Print Button -->
                    <button onclick="printPDF('${directUrl}')"
                            class="hidden md:flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-500 to-cyan-600 text-white rounded-xl font-bold text-sm hover:from-blue-600 hover:to-cyan-700 active:scale-95 transition-all duration-200 shadow-md hover:shadow-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <!-- Share Button -->
                    <button onclick="sharePDF('${title}', '${directUrl}')"
                            class="hidden md:flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-purple-500 to-pink-600 text-white rounded-xl font-bold text-sm hover:from-purple-600 hover:to-pink-700 active:scale-95 transition-all duration-200 shadow-md hover:shadow-lg">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- PDF Loading State -->
            <div id="pdf-loading" class="flex-1 bg-gradient-to-br from-slate-800 to-slate-900 flex flex-col items-center justify-center">
                <div class="text-center space-y-6">
                    <!-- Animated PDF Icon -->
                    <div class="relative">
                        <div class="w-24 h-32 bg-gradient-to-br from-blue-500 to-cyan-600 rounded-lg mx-auto shadow-2xl flex items-center justify-center animate-pulse">
                            <svg class="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                            </svg>
                        </div>
                        <div class="absolute -bottom-2 -right-2 w-10 h-10 bg-gradient-to-r from-yellow-400 to-amber-500 rounded-full flex items-center justify-center text-xs font-black text-white shadow-lg">
                            PDF
                        </div>
                    </div>
                    ...
                    <div class="space-y-3">
                        <div class="w-64 h-2 bg-slate-700 rounded-full overflow-hidden">
                            <div id="progress-bar" class="h-full bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full animate-progress"></div>
                        </div>
                        <p class="text-slate-300 font-medium">PDF লোড হচ্ছে...</p>
            ...
            <div id="pdf-viewer-container" class="flex-1 bg-slate-900 hidden">
                <div class="h-full relative">
                    <!-- Viewer Controls -->
                    <div id="viewer-controls" class="absolute top-4 right-4 z-20 flex flex-col gap-2 bg-white/90 backdrop-blur-sm p-3 rounded-xl shadow-lg border border-slate-200">
            ...
            <div id="pdf-footer" class="hidden bg-white/95 backdrop-blur-sm border-t border-slate-200 p-3 flex justify-between items-center">
                <div class="text-xs text-slate-600">
                    <span id="current-page">Page 1</span> 
                    <span class="mx-2">•</span>
                    <span id="total-pages">Loading...</span>
                </div>
                <div class="flex items-center gap-4">
                    <button onclick="prevPage()" 
                            class="p-2 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
...
/**
 * Process PDF URL to extract proper viewing and download URLs
 */
function processPDFUrl(url) {
    ...
    if (url.includes("drive.google.com")) {
        ...
        if (fileId) {
            // Enhanced Google Drive Viewer with PDF.js fallback
    ...
    // Direct PDF URL
        ...
        // Use PDF.js viewer for direct PDFs
        viewUrl = `https://mozilla.github.io/pdf.js/web/viewer.html?file=${encodeURIComponent(url)}`;
        downloadUrl = url;
    }
    // Other URLs - use Google Docs viewer as fallback
    else {
        viewUrl = `https://docs.google.com/viewer?embedded=true&url=${encodeURIComponent(url)}`;
        downloadUrl = url;
    }

    return { viewUrl, downloadUrl, directUrl };
}

/**
 * PDF Load Complete Handler
 */
function onPDFLoad() {
    setTimeout(() => {
        ...
        showToast("PDF সফলভাবে লোড হয়েছে!", "success");
 ...
 */
function prevPage() {
    // Implementation depends on PDF viewer API
...
function zoomIn() {
    ...
    // PDF viewer zoom implementation
    console.log("Zoom in");
}
...
function toggleFullscreen() {
    ...
    if (!document.fullscreenElement) {
        container.requestFullscreen().catch(err => {
            showToast("Fullscreen সমর্থিত নয়", "error");
        });
    } else {
...
function printPDF(url) {
    ...
    if (printWindow) {
        ...
        showToast("প্রিন্ট উইন্ডো খুলতে ব্যর্থ!", "error");
    }
}
...
function sharePDF(title, url) {
    if (navigator.share) {
        navigator.share({
            title: title,
            text: 'Dhumketu Educational Hub - PDF Document',
            url: url
        }).then(() => {
            showToast("সফলভাবে শেয়ার করা হয়েছে!", "success");
        }).catch(error => {
...
function trackPDFView() {
    ...
    console.log("PDF viewed - tracked");
}

function trackPDFDownload(title) {
    // Implement download tracking here
...
function showToast(message, type = "info") {
    ...
    const existingToast = document.getElementById('pdf-toast');
    ...
    toast.id = 'pdf-toast';
    toast.className = `fixed top-4 right-4 z-[10000] ${colors[type]} text-white px-6 py-3 rounded-xl shadow-2xl font-bold text-sm animate-slide-in`;
...
function addPDFViewerStyles() {
    ...
    const styles = `
        ...
        #pdf-viewer-container:hover #pdf-viewer-controls {
            opacity: 1;
        }
