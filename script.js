...
function updateLiveClock() {
    ...
    // ক্লক এলিমেন্ট আপডেট
    const noticeClock = document.getElementById('notice-clock');
    if (noticeClock) {
        noticeClock.textContent = timeString;
    }
}

// প্রতি সেকেন্ডে ক্লক আপডেট
setInterval(updateLiveClock, 1000);

// ৪. নোটিশ মার্কি সিস্টেম (সব নোটিশ একই লাইনে)
function setupNoticeMarquee() {
    const noticeEl = document.getElementById('notice-marquee');
    if (!noticeEl) return;
    
    // সব নোটিশ লোড
    db.ref('notices').orderByChild('timestamp').limitToLast(20).once('value')
        .then(snap => {
            ...
            if (notices && Object.keys(notices).length > 0) {
                ...
                // সময়ের ক্রমে সাজানো (নতুন থেকে পুরনো)
                allNotices.sort((a, b) => b.timestamp - a.timestamp);
                
                // সব নোটিশ এক লাইনে জোড়া লাগানো
                const allNoticeText = allNotices.map(notice => `📢 ${notice.text}`).join(' ••• ');
                noticeEl.textContent = allNoticeText;
                
                // মার্কি অ্যানিমেশন শুরু
                startMarqueeAnimation(noticeEl);
            } else {
                noticeEl.textContent = "📢 Dhumketu Educational Hub এ আপনাকে স্বাগতম! নতুন নোটিশের জন্য অপেক্ষা করুন...";
            }
        })
        .catch(err => {
            console.error("Error loading notices:", err);
            noticeEl.textContent = "📢 Dhumketu Educational Hub - Error loading notices";
        });
}
